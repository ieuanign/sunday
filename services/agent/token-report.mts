...
/** One-line headline (the Logger's, and any notifier's): totals by class + peak
 *  ctx/zone + top-3 consumers by weighted rank. */
...
/** Persist a report into `dir`: `<runId>.{json,md}` + a one-line summary appended to
 *  `history.jsonl`. Takes the directory it writes to rather than reading one out of
 *  the environment — the same reason every Logger destination does (services/
 *  destinations.mts), and what keeps a smoke out of the real `var/`. Makes its own
 *  directory: `lib/paths.mts` only NAMES paths. */
 ...
 *  store the report under that repo's durable log dir, and log the headline. NEVER
 *  throws — an accounting report must not break a run that already succeeded; a
 *  report that could not be produced is reported as info, because nothing about the
 *  run itself has failed. Called by the agent implementation once the run returns. */
export function emitReport(log: ModuleLogger, fullName: string, sessionFilePath: string, runId: string): void {
  try {
    const report = buildReport(collectRows(sessionFilePath), runId);
    writeReport(tokenReportDir(fullName), report);
    log.info(headline(report), { repo: fullName });
  } catch (err) {
    log.info(`token-report skipped: ${err instanceof Error ? err.message : String(err)}`, { repo: fullName });
  }
}
