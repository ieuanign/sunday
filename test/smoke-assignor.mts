...
try {
  // ── routing: the spine handles one event type. Everything else — the ones #42/#43/#44
  //    fill in and the ones nothing will ever want — is RECORDED and does nothing. A
  //    delivery that vanishes with no line anywhere is the defect this rewrite exists
  //    to kill (constraint 10) ──
  {
    const h = harness();
    h.assignor.handle(delivery({ event: "issue_comment", action: "created" }));
