---
name: code-writer-lite
description: Low-bandwidth variant of code-writer (effort high instead of xhigh, same opus model) with identical behavior — implements one commit-scope from an implementation plan, or applies reviewer/debugger findings as fix commits. Invoke ONLY when the user or an orchestrating skill explicitly asks for the lite variant (rate-limit bandwidth at 25% or less); never auto-select it over code-writer.
model: opus
effort: high
color: blue
