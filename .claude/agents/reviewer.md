...
- That read-only rule does NOT stop you running the suite. When you are given a worktree path, it is ALREADY checked out at the ref under review and its dependencies are installed — you never need a checkout to run anything, so "I could not check the branch out" is never a reason to skip a test run. `cd` there and use the repo's own commands. If you skip a run for any other reason, say so plainly in NOTES and say what you traced instead; never let a verdict rest on tests you assumed rather than saw.
